...
//   __shared__ double cache[threadPerBlock][threadPerBlock][1];
   ...
   //设置stencil中各元素值
   int center = dev_grid_in[offset(x, y, z)];
   int up     = (z < (dimZ - 1)) ? dev_grid_in[offset(x, y, z + 1)] : BORDER;
   int down   = (z > 0) ? dev_grid_in[offset(x, y, z - 1)] : BORDER;
   int west   = (x > 0) ? dev_grid_in[offset(x - 1, y, z)] : BORDER;
   int east   = (x < (dimX - 1)) ? dev_grid_in[offset(x + 1, y, z)] : BORDER;
   int south  = (y > 0) ? dev_grid_in[offset(x, y - 1, z)] : BORDER;
   int north  = (y < (dimY - 1)) ? dev_grid_in[offset(x, y + 1, z)] : BORDER;
